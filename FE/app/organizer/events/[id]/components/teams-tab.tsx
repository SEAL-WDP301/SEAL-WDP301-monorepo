...
export default function TeamsTab({ event }: { event: any }) {
  ...
  const [bulkActionType, setBulkActionType] = useState<"approved" | "rejected">(
    "approved",
  );
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [selectedTeamForDetails, setSelectedTeamForDetails] = useState<
    any | null
  >(null);
  ...
  const isRoundEnded = currentRound
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => {
      enqueueSnackbar("Team status updated successfully", {
        variant: "success",
      });
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "pending":
        return (
          <span className="px-2 py-1 bg-amber-500/10 text-amber-500 rounded-md text-xs font-semibold">
            Pending
          </span>
        );
      case "approved":
      ...
      default:
        return (
          <span className="px-2 py-1 bg-muted text-muted-foreground rounded-md text-xs font-semibold">
            {status}
          </span>
        );
    }
  ...
  const bulkStatusMutation = useMutation({
    mutationFn: async ({
      ...
      return axiosClient.post(`/organizer/teams/bulk-status`, {
        teamIds,
        status,
        reason,
      });
    ...
    onSuccess: () => {
      enqueueSnackbar("Teams status updated successfully", {
        variant: "success",
      });
      queryClient.invalidateQueries({ queryKey: ["organizerTeams", event.id] });
      setSelectedTeamIds([]);
    ...
    onError: (error: any) => {
      enqueueSnackbar(
        error.response?.data?.message || "Failed to update teams status",
        { variant: "error" },
      );
    },
  });
  ...
  const currentTeamDetails =
    teams?.find((t: any) => t.id === selectedTeamForDetails?.id) ||
              ...
              filteredTeams.map((team: any, idx: number) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 font-medium text-foreground">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {roundId &&
                        ...
                        (() => {
                          // eslint-disable-next-line @typescript-eslint/no-explicit-any
                          const tr = team.teamRounds.find(
                            (r: any) => r.roundId === Number(roundId),
                          );
                          if (tr?.status === "advanced") {
                            return (
                              <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-500 rounded text-[10px] font-bold border border-emerald-500/20 whitespace-nowrap">
                                PASSED
                              </span>
                            );
                          }
                          if (tr?.status === "eliminated") {
                            return (
                              <span className="px-2 py-0.5 bg-red-500/10 text-red-500 rounded text-[10px] font-bold border border-red-500/20 whitespace-nowrap">
                                ELIMINATED
                              </span>
                            );
                          }
                  ...
                  <td className="px-6 py-4">
                    {team.members?.filter(
                      (m: any) =>
                        m.status === "accepted" || m.role === "leader",
                    ).length || 0}{" "}
                    ...
                    team.mentorAssignments.length > 0 ? (
                      <div className="flex flex-wrap gap-1">
                        {team.mentorAssignments.map((ma: any) => (
                          <span
                            key={ma.mentorId}
                            className="px-2 py-1 bg-amber-500/10 border border-amber-500/20 text-amber-600 rounded text-xs font-semibold whitespace-nowrap"
                          >
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    ...
                    {team.status === "pending" && !isRoundEnded && (
                      <>
                        <Button
                          ...
                          onClick={() =>
                            handleUpdateStatus(team.id, "approved")
                          }
                    ...
                    {team.status === "approved" && (
                      <Button
                        ...
                        className="border-red-500/30 text-red-500 hover:bg-red-500/10"
                        onClick={() =>
                          handleUpdateStatus(team.id, "disqualified")
                        }
                        disabled={updateStatusMutation.isPending}
                      >
                    ...
                    {(team.status === "rejected" ||
                      ...
                      <span
                        className="text-xs text-muted-foreground italic max-w-[150px] inline-block truncate"
                        title={team.eliminationReason}
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-card border border-border p-6 rounded-2xl w-full max-w-md shadow-2xl">
            <h3 className="text-xl font-bold text-foreground mb-2">
              Reject / Disqualify Team
            </h3>
            <p className="text-sm text-muted-foreground mb-4">
              Please provide a reason for rejecting or disqualifying this team.
              This reason will be recorded and potentially visible to the team.
            </p>
            <textarea
            ...
            <div className="flex justify-end gap-3">
              ...
              <Button
                ...
                disabled={
                  updateStatusMutation.isPending || !eliminationReason.trim()
                }
              >
      ...
      <Dialog open={isBulkDeleteOpen} onOpenChange={setIsBulkDeleteOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete {selectedTeamIds.length} team(s)?
              This action will permanently remove these teams, their members,
              and all submissions.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="bg-muted/50 p-3 rounded-md max-h-[150px] overflow-y-auto text-foreground text-sm mt-4">
            <ul className="list-disc pl-4 space-y-1">
              {teams
                ?.filter((t: any) => selectedTeamIds.includes(t.id))
                .map((team: any) => (
                  // eslint-disable-line @typescript-eslint/no-explicit-any
                  <li key={team.id}>
      ...
      <Dialog open={isBulkStatusOpen} onOpenChange={setIsBulkStatusOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Confirm Bulk{" "}
              {bulkActionType === "approved" ? "Approval" : "Rejection"}
            </DialogTitle>
            <DialogDescription>
              You are about to mark {selectedTeamIds.length} team(s) as{" "}
              {bulkActionType}.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex justify-end gap-3 mt-4">
            ...
            <Button
              ...
              disabled={
                ...
                (bulkActionType === "rejected" && !bulkStatusReason.trim())
              }
            >
              {bulkStatusMutation.isPending
                ? "Processing..."
