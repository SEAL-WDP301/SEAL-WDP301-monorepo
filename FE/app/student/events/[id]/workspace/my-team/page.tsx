...
export default function TeamMembersPage() {
  ...
  const updateTeamMutation = useMutation({
    ...
    mutationFn: async (data: any) => {
      if (isReadOnly)
        throw new Error("This event has ended. The workspace is view only.");
    ...
    return (
      <div className="flex min-h-[50vh] flex-col items-center justify-center text-center">
        ...
        <p className="mt-2 text-muted-foreground max-w-md">
          You haven&apos;t joined or created a team for this event yet, or your
  ...
  const handleSendInvite = () => {
    if (isReadOnly) {
      enqueueSnackbar("This event has ended. The workspace is view only.", {
        variant: "warning",
      });
      return;
    }
      ...
      <header className="border-b border-border pb-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="text-sm font-medium uppercase tracking-[0.2em] text-orange-500 flex items-center gap-3">
              ...
              <Badge
                ...
                className={
                  isLeader
                    ? "bg-orange-500/10 text-orange-500 border-orange-500/20 pointer-events-none"
                    : "pointer-events-none"
                }
              >
                {isLeader ? (
                  <span className="flex items-center gap-1">
                    <Crown className="h-3 w-3" /> LEADER VIEW
                  </span>
                ) : (
                  "MEMBER VIEW"
                )}
            ...
            <p className="mt-2 text-sm text-muted-foreground max-w-2xl">
              View your team's roster, track pending invitations, and
              collaborate towards victory.
            </p>
            {team.mentorAssignments && team.mentorAssignments.length > 0 && (
              <div className="mt-5 flex items-center gap-3 p-3 rounded-xl bg-blue-500/5 border border-blue-500/20 w-fit pr-6">
                ...
                <div
                  onClick={() =>
                    router.push(`/student/events/${eventId}/workspace/mentor`)
                  }
                ...
                >
                  ...
                  <p className="text-sm font-medium text-foreground">
                    {team.mentorAssignments[0].mentor?.name}
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Badge
              variant="outline"
              className="px-4 py-2 bg-card border-border shadow-sm text-sm"
            >
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button
              variant="outline"
              size="sm"
              className="rounded-xl border-border bg-background h-10 px-4 pointer-events-none"
            >
      ...
      <div className="space-y-8">
        <div className="space-y-4">
          <h3 className="text-lg font-bold text-foreground px-1 flex items-center gap-2">
            Active Members
            <Badge variant="outline" className="ml-1 bg-muted/50">
