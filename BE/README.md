...
An enterprise-ready, high-concurrency **Backend API Engine** for **SEAL** (Software Engineering & AI-driven Hackathon Management Platform). Built with NestJS, Prisma ORM, Redis, BullMQ, Socket.IO with Redis Pub/Sub Adapter, and Winston Logger, this system automates end-to-end hackathon events—from GitHub organization repository provisioning and multi-channel notifications to multi-judge rubric scoring and statistical Inter-Rater Reliability telemetry.
...
**SEAL Backend** digitizes and automates this entire lifecycle across 4 role-based workspaces:

1. 👑 **Organizers:** Create multi-round events, define competition tracks and rubrics, automate GitHub private organization repository creation, trigger bulk email reminders, and monitor real-time analytics.
...
graph TD
    ...
    Client -->|WebSocket WSS| Ws_Gateway[Socket.IO Gateways: Admin & Feedback]
...
* **Domain Encapsulation without Network Overhead**: Microservices introduce significant network latency between service boundaries, distributed transaction complexity (Saga pattern), and operational overhead. A Modular Monolith keeps domain boundaries cleanly separated inside discrete modules (`auth`, `event`, `team`, `submission`, `judge`, `mentor`, `round`, `analytics`) while executing in a single unified runtime process.
* **Prepared for Future Microservice Extraction**: Each module encapsulates its own DTOs, services, controllers, and database access logic. If high traffic requires isolating a specific domain (e.g. `github` or `notification`), it can be extracted into an independent microservice with minimal refactoring.

---

## ⚡ Technical Features & Engineering Highlights

### 1. Multi-Pod WebSocket Scaling (`RedisIoAdapter`)
- **Integration**: Implemented custom `RedisIoAdapter` extending `@nestjs/platform-socket.io` and powered by `@socket.io/redis-adapter` over `ioredis`.
- **Function**: Enables real-time Socket.IO room messages (`user-${userId}`, `admin-event-${eventId}`, `admin-round-${roundId}`) to be broadcasted seamlessly across multiple scaled Kubernetes backend pods (`replicas: 2+`) via Redis Pub/Sub without sticky session requirements.
...
- **Function**: Replaces high-overhead database polling scripts (`@Cron`) with pure delayed jobs indexed by millisecond execution timestamps. When round deadlines are extended, existing delayed jobs (`bulk-reminder-15m-round-${id}` and `auto-freeze-round-${id}`) are cleanly removed from Redis and re-indexed with new delay timestamps with zero database query load.
...
| **Real-Time Adapter** | Socket.IO & `@socket.io/redis-adapter` | v4.8.3 | WebSockets with multi-pod Redis Pub/Sub adapter |
...
│   │   ├── analytics/          # Scoring analytics & Inter-Rater Reliability
│   │   ├── auth/               # Auth endpoints, JWT strategy, OTP verification
│   │   ├── event/              # Hackathon event & track management
│   │   ├── feedback/           # Mentor/Judge feedback gateways
│   │   ├── judge/              # Rubric evaluation matrix & scoring
│   │   ├── mentor/             # Mentor scheduling & team assignments
│   │   ├── notification/       # WebSocket notification gateways
│   │   ├── organizer/          # Organizer management endpoints
│   │   ├── registration/       # Student event registration
│   │   ├── round/              # Round automation queues & delayed processors
│   │   ├── student/            # Student workspaces & submission tracking
│   │   ├── submission/         # Team project submissions
│   │   ├── team/               # Team formation & invitations
